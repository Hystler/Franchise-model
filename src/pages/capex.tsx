  ...
  return (
    <Shell>
      ...
      <form className="band gridForm" method="post" action="/api/capex">
        ...
        <label>Оплачено до открытия<select name="paidBeforeOpening" defaultValue="true"><option value="true">Да</option><option value="false">Нет</option></select></label>
        <button className="primary" type="submit">Добавить CAPEX</button>
      </form>
      <section className="band">
        <h2>Инвестиции на открытие: {rub(totals.initialInvestment)} · Амортизация / мес: {rub(totals.monthlyDepreciation)}</h2>
        <table>
          <thead><tr><th>Статья</th><th>Сумма</th><th>Амортизация</th><th>Комментарий</th><th>Обязательно</th><th>До открытия</th><th>Действия</th></tr></thead>
          <tbody>
            {rows.map((row) => (
              <tr key={row.id}>
                <td colSpan={7}>
                  <form className="rowEditor" onSubmit={(event) => { event.preventDefault(); update(row.id, event.currentTarget); }}>
                    ...
                    <select name="required" defaultValue={String(row.required)}><option value="true">Да</option><option value="false">Нет</option></select>
                    <select name="paidBeforeOpening" defaultValue={String(row.paidBeforeOpening)}><option value="true">Да</option><option value="false">Нет</option></select>
